...
def addDevice(self, *args, **kwargs):
    ...
    # guard against null args or null kwargs
    if not args or not kwargs:
        return original(self, *args, **kwargs)

    import logging
    log = logging.getLogger('zen.ModelerService')
    log.info("args: %s, kwargs: %s" % (str(args), str(kwargs)))

    snmpCommunity = kwargs['snmpCommunity'] or ''
    snmpPort = kwargs['snmpPort'] or ''
    zCommandUsername = kwargs['zCommandUsername'] or ''
    zCommandPassword = kwargs['zCommandPassword'] or ''
    zWinUser = kwargs['zWinUser'] or ''
    zWinPassword = kwargs['zWinPassword'] or ''
    zProperties = kwargs['zProperties'] or ''
    model = kwargs['model'] or ''
    collector = kwargs['collector'] or ''
    title = kwargs['title'] or ''
    deviceName = args[0]
    deviceClass = args[1]
    manageIp = kwargs['manageIp'] or ''
    cProperties=kwargs['cProperties'] or ''
    rackSlot=kwargs['rackSlot'] or ''
    productionState=kwargs['productionState'] or ''
    comments=kwargs['comments'] or ''
    hwManufacturer=kwargs['hwManufacturer'] or ''
    hwProductName=kwargs['hwProductName'] or ''
    osManufacturer=kwargs['osManufacturer'] or ''
    osProductName=kwargs['osProductName'] or ''
    priority=kwargs['priority'] or ''
    tag=kwargs['tag'] or ''
    serialNumber=kwargs['serialNumber'] or ''
    locationPath=kwargs['locationPath'] or ''
    systemPaths=kwargs['systemPaths'] or ''
    groupPaths=kwargs['groupPaths'] or ''
